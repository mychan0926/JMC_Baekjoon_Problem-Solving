...
void max(int start, int end, int lims, int lime, int node)
{
	if (lime<start || lims>end || start > end)
	{
		return;
	...
	else
	{
		...
		max(start, mid, lims, lime, 2 * node);
		max(mid + 1, end, lims, lime, 2 * node + 1);
		return;
	}
...
int main()
{
	...
	s_max.resize(pow(2, floor), 0);
	for (int i = 0; i < size; i++)
	{
		s_min[i + floor_first_array] = index[i];
		s_max[i + floor_first_array] = index[i];
	}
